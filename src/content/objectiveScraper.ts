 ...
 */
export function isObjectiveResultPage(): {
  isResult: boolean;
  ...
  if (
    (url.includes("/reading") && url.includes("/result")) ||
    (url.includes("/score/") && /\breading\b/i.test(pageText))
  ) {
  ...
  if (
    (url.includes("/listening") && url.includes("/result")) ||
    (url.includes("/score/") && /\blistening\b/i.test(pageText))
  ) {
 ...
 */
export function scrapeObjectiveResult(
  ...
  try {
    ...
    const title =
      document.title ||
      `${type.charAt(0).toUpperCase() + type.slice(1)} Test Result`;
    ...
    // Extract raw score and platform feedback if available
    const rawScore = extractRawScore();
    const platformFeedback = extractPlatformFeedback();
    ...
    console.log(`${LOG_PREFIX} ${type} result scraped:`, {
      ...
      platformBand,
      rawScore,
      platformFeedbackLength: platformFeedback?.length || 0,
    });
...
function extractPlatformFeedback(): string | undefined {
  const selectors = [
    ".band-score-feedback",
    ".score-feedback",
    ".feedback",
  ...
  const match = bodyText.match(/Band Score[\s\S]{0,2500}/i);
  if (match) return cleanText(match[0]).slice(0, 3000);
